...
export default function Admin() {
  ...
  async function loadComments() {
    ...
    const { data, error: commentsError } = await supabase.from('comments').select('id, book_id, user_email, content, created_at').order('created_at', { ascending: false })
  ...
  async function deleteBook(book) {
    ...
    try {
      ...
      if (book.file_path && !storageItems.some(item => item.bucket === 'books' && item.path === book.file_path)) storageItems.push({ bucket: 'books', path: book.file_path })
  ...
  async function deleteComment(comment) {
    ...
    try {
      ...
      if (deleteError) {
        console.error('Suppression commentaire:', deleteError)
        setError(`Impossible de supprimer ce commentaire : ${deleteError.message}`)
        return
      }
      setComments(prev => Array.isArray(prev) ? prev.filter(item => item.id !== comment.id) : [])
    } catch (err) {
      console.error('Suppression commentaire:', err)
      setError(`Impossible de supprimer ce commentaire : ${err?.message || 'erreur inconnue'}`)
    } finally {
  ...
  const filteredComments = useMemo(() => {
    ...
    return comments.filter(item => !q || String(item.user_email || '').toLowerCase().includes(q) || String(item.content || '').toLowerCase().includes(q) || String(item.book_id || '').toLowerCase().includes(q))
    ...
    return (
      <main className="page"><div className="container">
        ...
        <form onSubmit={handleLogin} className="admin-password-card">
          <label htmlFor="admin-email">E-mail administrateur</label><input id="admin-email" type="email" value={email} onChange={e => setEmail(e.target.value)} autoComplete="username" required />
          <label htmlFor="admin-password">Mot de passe</label><input id="admin-password" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Mot de passe Supabase" autoComplete="current-password" required />
  ...
  return (
    <main className="page"><div className="container">
      ...
      <section className="admin-moderation"><div className="catalogue-heading"><div><p className="section-kicker">MODÉRATION</p><h2>Commentaires</h2></div><span className="result-count">{filteredComments.length} affiché{filteredComments.length > 1 ? 's' : ''}</span></div><input className="admin-comment-search" value={commentSearch} onChange={e => setCommentSearch(e.target.value)} placeholder="🔎 Rechercher un pseudo ou commentaire..." aria-label="Rechercher un commentaire" />
        {commentsLoading ? <div className="empty-card"><p>Chargement des commentaires...</p></div> : filteredComments.length === 0 ? <div className="empty-card"><h2>Aucun commentaire</h2><p>La modération est à jour.</p></div> : <div className="admin-comments-list">{filteredComments.map(item => { const relatedBook = books.find(book => book.id === item.book_id); return <article className="admin-comment-row" key={item.id}><div><strong>💬 {item.user_email || 'Lecteur'}</strong><span>{relatedBook?.title || 'Publication supprimée'}</span><p>{item.content}</p><small>{item.created_at ? new Date(item.created_at).toLocaleString('fr-FR') : ''}</small></div><button type="button" className="btn danger" onClick={() => deleteComment(item)} disabled={deletingComment === item.id}>{deletingComment === item.id ? 'Suppression...' : '🗑️ Supprimer'}</button></article> })}</div>}
      </section>
    </div></main>
  )
}
