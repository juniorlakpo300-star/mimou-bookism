...
export default function MimouIA() {
  ...
  return (
    <>
      ...
      {open && (
        <div style={{ position: 'fixed', right: '24px', bottom: '82px', zIndex: 99998, width: '390px', maxWidth: 'calc(100vw - 32px)', height: '620px', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid #334155', borderRadius: '20px', background: '#020617', color: '#fff', boxShadow: '0 25px 70px rgba(0,0,0,.55)' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
            {messages.map((item, index) => (
              <div key={`${item.role}-${index}`} style={{ maxWidth: '85%', marginLeft: item.role === 'user' ? 'auto' : 0, marginBottom: '12px', padding: '11px 13px', borderRadius: '14px', background: item.role === 'user' ? '#2563eb' : '#0f172a', border: item.role === 'user' ? 'none' : '1px solid #1e293b', lineHeight: 1.55, fontSize: '14px', whiteSpace: 'pre-wrap' }}>
                {item.content}
              </div>
            ))}
            ...
            {results.length > 0 && (
              <div style={{ marginTop: '18px' }}>
                <div style={{ color: '#94a3b8', fontSize: '11px', fontWeight: 800, letterSpacing: '1px', marginBottom: '9px' }}>LIVRES CORRESPONDANTS</div>
                {results.map((book) => (
                  <Link key={book.id} to={`/read/${book.id}`} style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '8px', padding: '9px', borderRadius: '12px', background: '#0f172a', border: '1px solid #1e293b', color: '#fff' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      {book.category && <div style={{ color: '#60a5fa', fontSize: '11px', marginTop: '2px' }}>{book.category}</div>}
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <form onSubmit={(event) => { event.preventDefault(); sendMessage() }} style={{ padding: '12px', borderTop: '1px solid #1e293b' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button type="submit" disabled={loading || !message.trim()} style={{ alignSelf: 'flex-end', border: 'none', borderRadius: '11px', padding: '10px 13px', background: '#2563eb', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>Envoyer</button>
            </div>
          </form>
        </div>
      )}
